# BWA Desktop 0.6.79 — guida all’aggiornamento

## Aggiornare un PC già configurato

Apri BWA e accetta l’aggiornamento proposto da Mini Matteo, oppure usa **Amministratore → Backup e aggiornamenti → Controlla aggiornamenti**. La 0.6.79 è cumulativa: non occorre installare le versioni intermedie. Il programma verifica firma e contenuto, conserva una copia dei file precedenti e riavvia BWA. Non reinstallare il setup sopra una cartella con dati.

Per una nuova postazione usa `BWA-Desktop-0.6.79-Setup.exe`. Scegli subito la lingua; il modello BWA iniziale è incluso. La preparazione di Ollama e del modello richiede Internet e diversi GB soltanto quando mancano. Ristorante, utilizzatori e accessi si configurano al primo avvio.
...
Sulla postazione ristorante apri **Amministratore → Assistenza e diagnostica → Consultazione da casa**. Se il PC è già attivato con codice BWA Assistenza, puoi abilitarla direttamente.

Per un PC con archivio già popolato e vecchia connessione GitHub, usa **Collega questa postazione con codice**. Da BWA Assistenza genera un normale codice di attivazione `BWA1` per lo stesso ristorante e inseriscilo in questa finestra. Il nome deve corrispondere a quello in Admin. Questo collegamento separato conserva archivio e connessione di backup: non cancellare dati e non ripetere l'installazione. Il codice `BWAV1` serve invece al PC che consulta.
...
Il client mostra lordo, netto, Petty Cash, coperti e scontrino medio delle chiusure salvate. Il riepilogo mensile include produttività, coperti e fatture nette registrate: queste ultime comprendono le fatture incluse, con rettifiche e note di credito, escludendo quelle eliminate o escluse. Non rappresentano tutti i costi né la percentuale Food Cost; gli ordini provvisori non sono conteggiati. Un trattino indica dati non disponibili, anche con una vecchia sorgente. Distingue data dei dati, pubblicazione e ultimo contatto. A PC ristorante spento restano gli ultimi dati validi, indicati come non recenti o non raggiungibili. Non sono vendite in tempo reale. Il client non esegue chiusure, bot, email, ripristini o modifiche. Gli accessi si revocano dalla stessa sezione Admin; una sola postazione sorgente è ammessa per ristorante.
...
## Frequenza dei controlli e modello BWA

In **Amministratore → Assistenza e diagnostica → Frequenza controlli automatici** puoi impostare gli intervalli in minuti: Outlook/ordini (predefinito 5), recensioni (10), Bomito (30) e pubblicazione della consultazione (2). Le funzioni disattivate restano disattivate. Gli intervalli sono cadenze di controllo, non promesse di completamento: dipendono dalla connessione e dai lavori in corso. Il contatto del PC di consultazione viene aggiornato separatamente ogni minuto.

Il **Modello BWA** prepara i dati in background e riutilizza il modello del mese finché l'archivio non cambia. Se cambi rapidamente mese o foglio, mostra l'ultima selezione richiesta. Le celle protette restano protette e le modifiche consentite continuano a essere salvate automaticamente.
